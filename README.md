...
### TASK: Schedule a job to run regularly

#### Instructions
 - Use the wall command to test outputting to all users
 - Use crontab -e to edit the crontab
 - Add wall "blah" on a schedule
 - Wait and see what happens

### TASK: Setup a git repository
...
#### Instructions:
 ...
 - cd git-testj
 - echo "blah" > content.txt
 - git add *
 - git commit -m "Initial commit."
 - git push
 - Clone on local machine
 - git clone brownbag@<IPAddress>:~/git-test.git
 - make a change and push
 - Pull the change from the server
 - Add a new user with no shell access
 - Create an ssh key for the new user
 - put the public key into ~/.ssh/authorized_keys
 - Try cloning the repo, making changes and pushing with the new users
...
#### Instructions
 ...
 - use fg to bring to the foreground
 - suspend the process again
 - use fg to run it in the background
 - use jobs command to show running jobs
...
Improve understanding of Linux directory structure
Improve understanding of piping and chaining
Learn something about crond
Learn something about linux file permssions
